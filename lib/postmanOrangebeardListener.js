...
class PostmanOrangebeardListener {
  ...
  onBeforeDone(err) {
    ...
    const numberOfSuitesStillActive = this.activeSuites.length;
    for (var i = 0; i < numberOfSuitesStillActive; i++) {
      this.finishSuite();
    }
  }
  ...
  getCurrentSuiteTempId() {
    return this.activeSuites.length ? this.activeSuites[this.activeSuites.length - 1].tempId : null;
  }
  ...
  finishSuite() {
    ...
    const currentSuiteStatus =
      this.activeSuites[this.activeSuites.length - 1].status || TestStatus.PASSED;
