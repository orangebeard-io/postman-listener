...
class OrangebeardReporter {
  constructor(newman, reporterConfig, collectionRunOptions) {
    ...
    console.log(`Listener config: ${JSON.stringify(this.orangebeardConfig)}`);
  ...
  onBeforeRequest(err, result) {
    ...
    this.consoleLogs?.forEach((log) => this.logMessage(test, log.message, log.level, log.time));
    this.consoleLogs = [];
  }
  ...
  finishTestSteps(err, result) {
    ...
    if (testWithError) {
      ...
      this.failAllStepsForTestId(testId, testWithError.error.message);
      this.activeTests[this.activeTests.length - 1].status = TestStatus.FAILED;
    }
  }
