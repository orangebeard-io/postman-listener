...
class OrangebeardReporter {
  ...
  finishStep(error, testAssertion) {
    ...
    this.clients.forEach((client, clientIndex) => {
      let stepIndex = this.activeSteps[clientIndex].findIndex(
      ...
      if (errorInfo) {
        this.logMessage(
          this.getCurrentTest(clientIndex),
          errorInfo.message,
          'ERROR',
          step.id,
          clientIndex,
        );
        this.activeTests[clientIndex][this.activeTests[clientIndex].length - 1].status =
          TestStatus.FAILED;
  ...
  onRequest(error, result) {
    this.clients.forEach((client, clientIndex) => {
      this.activeTests[clientIndex][this.activeTests[clientIndex].length - 1].response =
        result?.response;
  ...
  getCurrentSuite(clientIndex = 0) {
    return this.activeSuites[clientIndex].length
      ? this.activeSuites[clientIndex][this.activeSuites[clientIndex].length - 1].id
      : null;
  }
  ...
  getCurrentStep(clientIndex = 0) {
    return this.activeSteps[clientIndex].length
      ? this.activeSteps[clientIndex][this.activeSteps[clientIndex].length - 1].id
      : null;
  }
