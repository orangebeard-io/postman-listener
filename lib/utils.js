 ...
 * @type {String}
 */
const DEFAULT_TESTSET_NAME = 'Newman Test run';
...
module.exports = {
  ...
  matchPattern(str, patterns, index = 0) {
    ...
    const pattern = _.find(patterns, (p) => RegExp(p).exec(str));

    return pattern ? RegExp(pattern).exec(str)[index] : null;
  },
  ...
  getAttributesArrayFromConfigStr(stringAttr) {
    return stringAttr
  ...
  getAttributes(variables) {
    ...
    return attributes ? this.getAttributesArrayFromConfigStr(attributes.value) : [];
  },

  getTime() {
    const now = ZonedDateTime.now().withFixedOffsetZone().toString();
    return now;
  },
  ...
  getOrangebeardParameters(reporterConfig) {
    return {
      endpoint: reporterConfig.orangebeardIoOrangebeardEndpoint,
      token: reporterConfig.orangebeardIoOrangebeardToken,
      testset: reporterConfig.orangebeardIoOrangebeardTestset,
      project: reporterConfig.orangebeardIoOrangebeardProject,
      attributes: this.getAttributesArrayFromConfigStr(
        reporterConfig.orangebeardIoOrangebeardAttributes,
      ),
      description: reporterConfig.orangebeardIoOrangebeardDescription ?? undefined,
    };
  },

  getStartTestRun(orangebeardConfig = {}) {
    const combinedAttributes =
      orangebeardConfig.attributes ||
      this.getAttributesArrayFromConfigStr(orangebeardConfig.orangebeardIoOrangebeardAttributes);
    if (orangebeardConfig.ref_url !== undefined) {
      combinedAttributes.push({
        key: 'reference_url',
        value: orangebeardConfig.ref_url,
      });
    }

    return {
      testSetName:
        process.env.ORANGEBEARD_TESTSET || orangebeardConfig.testset || DEFAULT_TESTSET_NAME,
      description:
        orangebeardConfig.description || orangebeardConfig.orangebeardIoOrangebeardDescription,
  ...
  parseProjects(projectString) {
    if (!projectString) {
      return [];
    }
    return projectString.split(';').map(p => p.trim()).filter(p => p.length > 0);
  },
